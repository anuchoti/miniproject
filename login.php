...
<center><a class="nav-link" href="login1.php">VIEW FEEDBACK</a>
<a class="nav-link" href="login2.php">VIEW SELLERS</a>

<a class="nav-link" href="admin.php">LOGOUT</a></center>

                 <?php
                         require_once('includes/db.php');
                         $query="SELECT username FROM notes ORDER BY username";
                         $quer_run=mysqli_query($conn,$query);
                         $row=mysqli_num_rows($quer_run);
                         echo '<center><h1>Total no of users registered:' .$row.'</h1></center>';
$query="SELECT location FROM notes WHERE location='HYD'";
                         $quer_run=mysqli_query($conn,$query);
                         $row2=mysqli_num_rows($quer_run);
                         echo '<center><h3>HYD Location:' .$row2.'</h3></center>';
$query3="SELECT location FROM notes WHERE location='KODAD'";
                ...
                <div>
                    ...
                    <div class="note">
                        <div class="titleContainer">
                            ...
                            <div class="nt-links">
                                <a class="nt-link" href=<?php echo "delete.php?id=" . $note['id']; ?>>delete userinfo</a>
                            </div>                 
                        </div>
